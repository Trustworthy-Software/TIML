...
from dataloader import ExemplarIncrementalDataset, PureExemplarDataset, MultiExemplarDataset
...
class TIMLTrainer:
    ...
    def train(self, num_steps):
        ...
        for step in range(num_steps):
            ...
            if not self.config["il_trainer"] == "random" and os.path.exists(f"{trainer.save_path}/model_step_{step}.pkl"):
    ...
    def evaluate(self, trainer, val_dataloader, mapping_dict, inverse_mapping_dict, step, incremental_nbr_new_classes, indices_test_end_step):
        ...
        # overall_accuracy = len([i for i in range(len(labels)) if labels[i] == preds[i]])/len(labels) 
...
if __name__ == "__main__":
    ...
    import os
    from model import CILNet, MalNetTIML
    import json
    import shutil
    import glob
