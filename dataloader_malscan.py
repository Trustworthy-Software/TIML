...
class IncrementalDataset(Dataset):
        ...
        if self.mode == 'both': 
            for step in range(self.step+1):
                for class_name in self.data[f'step={step}']:
                    ...
                    indices.extend([(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'),
                                     self.families_global_indices[class_name]) for sha in self.data[f'step={step}'][class_name] if os.path.exists(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'))])
            ...
            for class_name in self.data[f'step={self.step}']:
                ...
                indices.extend([(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'),
                                     self.families_global_indices[class_name]) for sha in self.data[f'step={self.step}'][class_name] if os.path.exists(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'))])
    ...
    def __getitem__(self, index):
        vect_path, label = self.indices[index]
        vector = scipy.sparse.load_npz(vect_path).todense().ravel().tolist()[0]
        vector = torch.tensor(vector, dtype=torch.float)
        if self.config["malscan_vector"]:
            img = vector
        else:
            vector = torch.cat((vector.float(), torch.zeros(4984)))#196608-108664=87944
            img = torch.reshape(vector, (3, 176, 176))
...
class PureExemplarDataset(ExemplarIncrementalDataset):
    ...
    def __getitem__(self, index):
        ...
        vect_path, label = self.exemplars[index_new]
        vector = scipy.sparse.load_npz(vect_path).todense().ravel().tolist()[0]
    ...
    def update_data_indices(self):
        ...
        for class_name in self.data[f'step={self.step}']:
            ...
            if self.step > 0 and self.families_global_indices[class_name] in self.cls_id_list_so_far: 
                self.new_samps_of_old_families.extend([(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'), 
                                 self.families_global_indices[class_name]) for sha in self.data[f'step={self.step}'][class_name] if os.path.exists(os.path.join(self.path_vectors, 'malscanco', sha+'_malscanco.npz'))])
